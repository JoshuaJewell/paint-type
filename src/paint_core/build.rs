...
//
// Cargo build script for paint_core.
//
...
fn main() {
    ...
    if !static_lib_exists(&lib_dir) {
        println!(
            "cargo:warning=paint_core: libpt static library not found in {}; \
...
fn try_build_zig() {
    ...
    match status {
        Ok(s) if s.success() => {
            println!("cargo:warning=paint_core: built libpt via `zig build`");
        }
        Ok(s) => {
            println!(
                "cargo:warning=paint_core: `zig build` exited with status {}",
                s
            );
        }
        Err(e) => {
            println!(
                "cargo:warning=paint_core: could not run `zig build` ({}); \
