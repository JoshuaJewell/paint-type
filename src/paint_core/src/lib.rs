...
//
// Paint Core — the Rust side of the paint.type tile primitive.
//
...
//! Paint Core: the native image core for paint.type.
//!
...
/// Region renderer: flatten the visible layer stack into RGBA8 for display.
pub mod render;

/// Number of f16 scalars in one tile's pixel buffer (`TILE_SIZE^2 * 4`).
/// This is the element count for `Tile::read_buffer` / `write_buffer`.
pub const TILE_SCALARS: usize = (TILE_SIZE as usize) * (TILE_SIZE as usize) * 4;
...
unsafe extern "C" {
    ...
    fn pt_tile_read_buffer(tile_ptr: u64, out_ptr: u64) -> u32;
    fn pt_tile_write_buffer(tile_ptr: u64, in_ptr: u64) -> u32;
...
impl Tile {
    ...
    /// Bulk-read the entire pixel buffer (`TILE_SCALARS` f16 bit patterns,
    /// RGBA per pixel, row-major) in a single FFI crossing. Hot paths that
    /// touch many pixels must use this rather than per-pixel reads, which
    /// pay the FFI cost once per pixel.
    pub fn read_buffer(&self, out: &mut [u16; TILE_SCALARS]) -> Result<(), TileError> {
        // SAFETY: out is a live TILE_SCALARS-element array on the caller's
        // frame; pt_tile_read_buffer copies exactly that many scalars.
        let code = unsafe { pt_tile_read_buffer(self.raw, out.as_mut_ptr() as u64) };
        if code == RESULT_OK {
            Ok(())
        } else {
            Err(TileError::from_code(code))
    ...
    /// Bulk-write the entire pixel buffer. Counterpart to [`Tile::read_buffer`].
    pub fn write_buffer(&self, data: &[u16; TILE_SCALARS]) -> Result<(), TileError> {
        // SAFETY: data is a live TILE_SCALARS-element array; pt_tile_write_buffer
        // copies exactly that many scalars into the tile.
        let code = unsafe { pt_tile_write_buffer(self.raw, data.as_ptr() as u64) };
        if code == RESULT_OK {
            Ok(())
        } else {
            Err(TileError::from_code(code))
