...
= TOPOLOGY.adoc — Generation Guide
Joshua Jewell (JoshuaJewell) <paint-type@pm.me>
:toc:
:sectnums:

== What Is TOPOLOGY.adoc?
...
[source,text]
----
Read the entire repository and produce a TOPOLOGY.adoc file for the repo root.
...
Use the template at TOPOLOGY.adoc in paint-type as a structural reference,
...
[source,bash]
----
cp /path/to/paint-type/TOPOLOGY.adoc ./TOPOLOGY.adoc
...
TOPOLOGY.adoc complements but does not replace:
...
TOPOLOGY.adoc is the _visual summary_ for humans; the a2ml files are the
