...
// `binary` default so that a structurally valid envelope still decodes when a
// field is absent; the backend decides how to treat an empty name. `binary`
// is a JSON number on the wire, which the old hand-rolled scanner could never
// read, so this is also the fix for the dead binary path.
