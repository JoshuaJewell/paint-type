...
// Capture buffer for the payload the handler actually receives.
var captured_payload: [512]u8 = undefined;
var captured_len: usize = 0;
var captured_fired: bool = false;

/// IPC callback that records the payload it was handed, so the test can assert
/// the structured payload survived the envelope round-trip rather than arriving
/// as an empty object (the symptom this fix addresses).
fn echoCaptureCallback(payload: [*:0]const u8, _: ?*anyopaque) callconv(.c) [*:0]const u8 {
    const slice = std.mem.span(payload);
    const n = @min(slice.len, captured_payload.len);
    @memcpy(captured_payload[0..n], slice[0..n]);
    captured_len = n;
    captured_fired = true;
    return "{\"ok\":true}";
}
...
test "display: structured IPC payload round-trips to the handler intact" {
    ...
    // budget (~2s) is spent.
    var waited: usize = 0;
    while (!captured_fired and waited < 500) : (waited += 1) {
    ...
    const got = captured_payload[0..captured_len];
